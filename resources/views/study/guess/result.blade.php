...
<div style="margin: 0px auto;">
        <table style="width: 100%;border: #d4d4d4 1px solid">
            ...
            @if(!empty($first))
            <tr><td style="width: 300px;text-align: center;">你得竞猜：
                    {{$info['team_a']}}
                    <font color="#ff0000">@if($first->g_result ==1 ) 胜 @elseif($first->g_result == 2) 平 @else 负 @endif</font>
                    {{$info['team_b']}}
                </td></tr>
            <tr><td style="width: 300px;text-align: center;">结果：
                    @if($first->g_result == $info['result'])恭喜你猜中啦 @else 很抱歉没猜中 @endif
                    </td></tr>
                @else
            <tr>
                <td style="width: 300px;text-align: center;">结果：你没有参与竞猜</td>
            </tr>
                @endif
        </table>
    {{--<a href="/study/guess/add">返回首页</a>--}}
    {{--<a href="/study/guess/list">返回列表</a>--}}
</div>
</body>
</html>
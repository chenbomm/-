...
class CreateJyGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_goods', function (Blueprint $table) {
            ...
            $table->enum('is_shop',[1,2])->default('1')->comment('是否上架 1，是 2，否');
            $table->timestamp('shop_time')->comment('上架时间');
            $table->enum('is_recommand',[1,2])->default('1')->comment('是否推荐 1，是 2，否');
            $table->enum('is_new',[1,2])->default('1')->comment('是否最新 1，是 2，否');
            $table->enum('is_hot',[1,2])->default('1')->comment('是否热销 1，是 2，否');
